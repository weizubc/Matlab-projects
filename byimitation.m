...
computation_count(1:T)=0;  % number of type 1 agents who meet with any one carrying good 3
computation_value(1:T)=0;  % number of type 1 agents who trade good 2 for good 3
pimt=0.09; % imitation probability
...
s=[0.1 0.25 0.3]; % storage cost with good 1, 2 and 3
u=[1.5 1.5 1]; % utility of consumption of good 1,2 and 3
d=[0.1 0.1 0.1]; % disutility of production of good 1,2 and 3     
...
% initial number of each type agents who hold good 1, 2, 3 at each period
...
for t=1:T   % outer 1 loop

    % record goods held by each type in each period
    for i=1:n
        if i<=n/3
    ...
    for i=1:2:(n-1)  % outer 2 loop
       ...
       %  record the goods agents hold before consumption
       ...
       % record goods agents hold at storage and their utility gain or loss in this round.
